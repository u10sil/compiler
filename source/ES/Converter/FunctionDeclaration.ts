...
addConverter<SyntaxTree.FunctionDeclaration>("functionDeclaration",
	(converter, node) => new ES.FunctionDeclaration(node.symbol, converter.convert(node.arguments), converter.getReturnType(node), Utilities.Enumerable.from(convertBody(converter, node.body ? node.body.statements : Utilities.Enumerable.empty)), node.tokens),
)
