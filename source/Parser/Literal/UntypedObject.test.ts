...
describe("Parser.Expressions.Literals.Object", () => {
	...
	it("literal multiple properties", () => {
		const parser = Parser.create("{ property0: 1337 property1: 42 }", handler)
		const statements = parser.fetch()!.statements.getEnumerator()
		const literal = statements.fetch()
		expect(literal instanceof SyntaxTree.Literal.UntypedObject).toBeTruthy()
		expect(SyntaxTree.filterId(literal!.serialize())).toEqual({ class: "literal.untypedObject", value: { property0: { class: "literal.number", value: 1337 }, property1: { class: "literal.number", value: 42 } } })
	})
	it("typed literal", () => {
		...
		expect(SyntaxTree.filterId(literal!.serialize())).toEqual({ class: "literal.typedObject", name: { class: "type.identifier", name: "Class" }, value: { class: "literal.untypedObject", value: { property: { class: "literal.number", value: 1337 } } } })
	})
})
