...
export class SeparatorTest extends Unit.Fixture {
	constructor() {
		...
		this.add("isSeparator()", () => {
			...
			this.expect(separator1.serialize(), Is.equal.to({ class: "separator", symbol: "" }))
			this.expect(separator2.isSeparator())
			this.expect(separator2.isSeparator("::"), Is.false)
			this.expect(separator2.serialize(), Is.equal.to({ class: "separator", symbol: ":" }))
		})
		this.add("scan separators", () => {
			...
			this.expect(token!.serialize(), Is.equal.to({ class: "separator", symbol: "." }))
			this.expect((token = Tokens.Separator.scan(source)) instanceof Tokens.Separator)
			this.expect((token as Tokens.Separator).symbol, Is.equal.to(":"))
			this.expect(token!.serialize(), Is.equal.to({ class: "separator", symbol: ":" }))
			this.expect((token = Tokens.Separator.scan(source)) instanceof Tokens.Separator)
